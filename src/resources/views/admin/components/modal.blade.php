...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body drop-zone" id="dropZone">
                ...
                <div class="media-header">
                    <div id="progress-container" style="width: 100%; background: #f3f3f3; height: 30px;     position: relative; margin-bottom: 10px; ">
                    ...
                    <span class="btn btn-success fileinput-button">
                        ...
                        <input id="file" type="file" name="images[]" multiple accept="image/jpeg,image/jpg,image/png,image/gif,image/svg+xml,video/mp4,video/webm,video/ogg,video/quicktime,video/x-msvideo,application/pdf">
                    </span>

                    <span class="btn btn-success fileinput-button delete_image" id="image-delete">
                        <i class="fa fa-trash-o" aria-hidden="true"></i>
                        <span>Hammasini o'chirish</span>
                    </span>

                    <span class="btn btn-success fileinput-button delete_image_select" id="image-delete-btn" style="cursor: pointer;">
                        <i class="fa fa-trash-o" aria-hidden="true"></i>
                        ...
                        @isset($menu)
                            ...
                            @foreach($menu->images as $image)
                                ...
                                <li class="thumb {{ $image->main ? 'main' : '' }}" data-id="{{ $image->id }}">
                                    @if($isVideo)
                                        <a href="{{ asset('storage/' . $image->image) }}" data-fancybox="gallery" class="thumbnail fancybox tooltips" rel="group">
                                    ...
                                    @else
                                        <a href="{{ asset('storage/' . $image->image) }}" class="thumbnail fancybox tooltips" data-fancybox="gallery" rel="group">
    ...
    <input id="updateMainButton_route" value="{{route('updateMainButton')}}">
</div>
...
<script>
    ...
    !function () {
        ...
        var ext = ['jpg', 'jpeg', 'png', 'gif', 'svg', 'mp4', 'webm', 'ogg', 'mov', 'avi', 'pdf'];
        ...
        function check(files) {
            ...
            if (bad.length) {
                alert('Недопустимые файлы:\n' + bad.join('\n') + '\n\nРазрешены: jpeg, jpg, png, gif, svg, mp4, webm, ogg, mov, avi, pdf');
                return 0;
            }
            return 1;
        }
...
if ($id) {
    ...
    filepart.addEventListener('change', function (event) {
        ...
        filepart.files = storedFiles.files;

        imageDropFiles(newFiles);
    });
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const sortable = new Sortable(mediaList, {
            animation: 200,
            handle: '.move',
        ...
        function updateLocalOrder() {
            ...
            const items = document.querySelectorAll('#media_list .thumb');
            let orderedNames = [];

            items.forEach(item => {
                if (item.dataset.name) {
                    orderedNames.push(item.dataset.name);
                }
            });

            document.getElementById('image_order').value = JSON.stringify(orderedNames);

            let newDataTransfer = new DataTransfer();

            orderedNames.forEach(name => {
                for (let i = 0; i < storedFiles.files.length; i++) {
                    if (storedFiles.files[i].name === name) {
                        newDataTransfer.items.add(storedFiles.files[i]);
                    }
                }
            });

            storedFiles = newDataTransfer;
            document.getElementById('file').files = storedFiles.files;

            console.log("Create tartib:", orderedNames);
        }

        //  EDIT MODE
        function updateImageOrderServer() {
            ...
            items.forEach((item, index) => {
                ...
                if (item.dataset.id && !item.classList.contains('uploading')) {
                    order.push({
                        id: item.dataset.id,
            ...
            fetch("{{ route('updateImageOrder') }}", {
                ...
                .then(data => {
                    if (!data.success) {
                        ...
                        console.log('Edit order updated ✔️');
                    }
                })
                .catch(err => {
