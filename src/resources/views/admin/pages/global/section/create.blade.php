    ...
    <div class="app-content content">
        ...
        <div class="content-wrapper ">
                        ...
                        <div class="col-12">
                            ...
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="{{route('admin.pages.section.index', ['slug' => $slug, 'id' => $id])}}">Bo'lim</a>
                                    </li>
            ...
            <div class="content-body">
                ...
                <section class="langs-edit">
                    ...
                    @endphp
                    <form action="{{ $isEdit ? route('admin.pages.section.update', $params1) : route('admin.pages.section.store', $params2) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @if($isEdit)
                            @method('PUT')
                        @endif
                                    ...
                                    @foreach($languages as $k => $language)
                                        <li class="nav-item fx align-items-center">
                                            <a class="nav-link {{ $k == 0 ? 'active' : '' }}" data-toggle="tab" href="#lang{{ $language->code }}">
                                                <img src="{{ sectionImages($language, true) }}" class="size_small" alt="flag">
                                                {{ $language->name }}
                                            </a>
                                        </li>
                                    @endforeach
                                                ...
                                                @if($setting->is_translatable)
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        @if($setting->type == 'textarea' || $setting->type == 'textarea-editor')
                                                            <textarea class="form-control {{$setting->type == 'textarea-editor' ? 'summernote' : ''}}" name="fields[{{ $language->code }}][{{ $setting->key }}]">{{ $value }}</textarea>
                                                        @elseif($setting->type == 'datetime-local')
                                                            <input type="datetime-local" class="form-control" name="fields[{{ $language->code }}][{{ $setting->key }}]" value="2025-06-01T13:30">
                                                        @elseif($setting->type == 'select')
                                                            <select class="form-control" name="fields[{{ $language->code }}][{{ $setting->key }}]" >

                                                                @foreach(json_decode($setting->options, true) as $item)
                                                                    <option value="{{$item}}" @if($item==$value) selected @endif>{{$item}}</option>
                                                                @endforeach

                                                            </select>
                                                        @else
                                                            @php $type = $setting->type == 'text' ? 'text' : 'number' @endphp
                                                            <input type="{{$type}}" class="form-control {{$setting->key == 'title' ? 'title' : ''}}" name="fields[{{ $language->code }}][{{ $setting->key }}]" value="{{ $value }}">
                                                        @endif
                                                    </div>
                                                @endif
                                            @endforeach
                            ...
                            <div class="col-12">
                                ...
                                @foreach($settings as $setting)
                                    @if(!$setting->is_translatable)
                                        ...
                                        @endphp
                                        <div class="form-group">
                                            @if($setting->selection_type != 'multiple')<label>{{ $setting->label }}</label>@endif
                                            @if($setting->type == 'relation')
                                                @if($setting->selection_type == 'single')

                                                    @php
                                                        $relationValue = old(
                                                            'relations.' . $setting->key .'id',
                                                            $isEdit ? ($section->relation_data[$setting->key]['id'] ?? null) : null
                                                        );
                                                    @endphp



                                                        <select class="form-control" name="relations[{{ $setting->key }}][id]">
                                                            <option value="">Tanlang</option>

                                                            @foreach($relations[$setting->key] ?? [] as $item)
                                                                <option value="{{ $item->id }}"
                                                                        @if($item->id == $relationValue) selected @endif>
                                                        ...
                                                        <input type="hidden"
                                                               name="relations[{{ $setting->key }}][table]"
                                                               value="{{ $setting->relation_table_name }}">

                                                @endif
                                                @if($setting->selection_type == 'multiple')

                                                    @if(isset($relations[$setting->key]['category']))
                                                        <input type="hidden"
                                                               name="relations[{{ $setting->key }}][category]"
                                                               value="{{ $relations[$setting->key]['category'] }}">
                                                    @endif

                                                    @if(isset($relations[$setting->key]['menu_id']))
                                                        <input type="hidden"
                                                               name="relations[{{ $setting->key }}][menu_id]"
                                                               value="{{ $relations[$setting->key]['menu_id'] }}">
                                                    @endif

                                                @endif
                                            @elseif($setting->type == 'textarea' || $setting->type == 'textarea-editor')
                                                <textarea class="form-control {{$setting->type == 'textarea-editor' ? 'summernote' : ''}}" name="fields[{{ $setting->key }}]">{{ $value2 }}</textarea>
                                            @elseif($setting->type == 'datetime-local')
                                                <input type="datetime-local" class="form-control" name="fields[{{ $setting->key }}]" value="2025-06-01T13:30">
                                            @elseif($setting->type == 'select')
                                                <select class="form-control" name="fields[{{ $setting->key }}]"  required>

                                                    @foreach(json_decode($setting->options, true) as $item)
                                                        <option value="{{$item}}" @if($item==$value2) selected @endif>{{$item}}</option>
                                                    @endforeach

                                                </select>
                                            @else
                                                @php $type = $setting->type == 'text' ? 'text' : 'number' @endphp
                                                <input type="{{$type}}" class="form-control {{$setting->key == 'title' ? 'title' : ''}}" name="fields[{{ $setting->key }}]" value="{{$value2 }}">
                                            @endif
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-12 mb-1 {{!in_array($id, $settingValues) ? 'd-none' : ''}}">
                                        ...
                                        <input type="text" class="form-control pickadate" name="publish_at" value="{{ old(
        'publish_at',
        ...
        : now()->format('Y-m-d')
                                    ...
                                    <div class="col-12 mb-1">
                                        ...
                                        <input type="text" class="form-control slug" name="slug" placeholder="slug" value="{{ old('slug', $section->slug ?? '') }}">
                                    </div>
                                    ...
                                    <div class="col-12">
                                        <div class="form-check">
                                            <div class="custom-control custom-switch custom-switch-success mr-2 mb-1">
                                                ...
                                                <input type="checkbox" class="custom-control-input" name="status" {{ old('status', $section->status ?? true) ? 'checked' : '' }} id="customSwitchStatus">
                                                <label class="custom-control-label" for="customSwitchStatus"></label>
                                            </div>

                                        </div>
                                    </div>
                                    @if(request()->route('slug')=='projects' && request()->route('parent_id')==null)
                                    <div class="col-12">
                                        <div class="form-check">
                                            <div class="custom-control custom-switch custom-switch-success mr-2 mb-1">
                                                <p class="mb-0">Mодал Статус</p>

                                                <input type="checkbox" class="custom-control-input" name="modal_status" {{ old('modal_status', $section->modal_status ?? false) ? 'checked' : '' }} id="customSwitchStatusmodal_status">
                                                <label class="custom-control-label" for="customSwitchStatusmodal_status"></label>
                                            </div>
