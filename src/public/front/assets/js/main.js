...
function initVideoShimmer(video) {
  if (!video || video.dataset.shimmerInit === 'true') return
  const wrapper = video.closest('.carousel__item--img') || video.parentElement
  if (!wrapper) return

  if (video.closest('.owl-item.cloned')) {
    wrapper.classList.remove('video-shimmer')
    return
  }

  video.dataset.shimmerInit = 'true'

  const removeShimmer = () => {
    wrapper.classList.remove('video-shimmer')
    video.removeEventListener('loadedmetadata', removeShimmer)
    video.removeEventListener('loadeddata', removeShimmer)
    video.removeEventListener('canplay', removeShimmer)
    video.removeEventListener('canplaythrough', removeShimmer)
    video.removeEventListener('error', removeShimmer)
  }

  wrapper.classList.add('video-shimmer')
  ...
  if (video.readyState >= 2) {
    ...
    video.addEventListener('loadedmetadata', removeShimmer)
    video.addEventListener('loadeddata', removeShimmer)
    video.addEventListener('canplay', removeShimmer)
    video.addEventListener('canplaythrough', removeShimmer)
    video.addEventListener('error', removeShimmer)
  }
}

function initProjectsHoverBorder() {
  const cards = document.querySelectorAll('.projects__grid--item, .partners__carousel--item')
  if (!cards.length) return

  const PROXIMITY = 120 // px — how far outside the card the effect starts fading in

  window.addEventListener('mousemove', e => {
    cards.forEach(card => {
      const rect = card.getBoundingClientRect()

      // Clamp mouse coords to card bounds to get the nearest point on the card
      ...
      if (dist < PROXIMITY) {
        // Express mouse position relative to the card (can be outside 0-100%)
        const x = Math.round(e.clientX - rect.left)
        const y = Math.round(e.clientY - rect.top)
        card.style.setProperty('--mouse-x', `${x}px`)
        card.style.setProperty('--mouse-y', `${y}px`)

        // Fade opacity based on distance: 1 when inside card, fades out toward PROXIMITY
        let opacity = dist === 0 ? 1 : 1 - dist / PROXIMITY
        opacity = opacity
        card.style.setProperty('--border-opacity', opacity)
...
$(window).on('load', function () { initProjectsHoverBorder() })
