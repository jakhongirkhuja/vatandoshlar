        ...
        try {
            ...
            if (isset($data['images'])) {
                ...
                foreach (array_reverse($data['images']) as $index=>$file) {
                    ...
                    $menuImage->compressed = (new PageSectionService())->resizeImage($file,'menu_images');
                    $menuImage->converted = (new PageSectionService())->convertImageToWebp($file,'menu_images');
...
//            }
    ...
    {
        $menus = MenuMain::find($id);
        if(!$menus){
            return back()->withErrors(['error' => 'Menu not found']);
        }
        if(PageSection::where('menu_main_id', $id)->exists()){
            return back()->withErrors(['error' => 'First remove Section insinde']);
        }
        try {
            ...
            PageSectionBlock::where('menu_main_id', $id)->delete();
            PageSectionSetting::where('menu_main_id', $id)->delete();
            $menus->delete();
